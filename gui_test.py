    ...
    def type_Check(self):
        ...
        try:
            ...
            if self.type_selector.currentText() == '취약' or self.type_selector.currentText() == '양호':
                ...
                for i in ['백신 및 실시간 감시','실시간 감시','','구글 Play 프로텍트','','개발자 옵션 활성화 여부','앱 권한 관리 (출처를 알 수 없는 앱)','']:
                    self.script_select.addItem(i)
        except Exception as e:
    ...
    def script_init(self):
        ...
        try:
                ...
                if reply == QMessageBox.Yes:
                    ...
                    script_temp = {
                        ...
                        "조치후_양호" : [
                            ...
                            "@_@",
                            "구글 Play 프로텍트가 활성화 되어있지 않아 취약하셨으나 해당부분 고객님께서 조치 원하셔서 조치 진행하였습니다.",
                            "@ㅁ@",
                            "개발자 옵션이 활성화 되어있어 취약하셨으나 고객님꼐서 조치 원해주셔서 조치 진행하였습니다.",
                            "앱 권한 관리 부분 중 출처를 알 수 없는 앱 설치권한이 부여되어있지않아 취약하셨으나,\n 고객님께서 조치 진행 해주셨습니다.",
                            "@ㅅ@",
                        ],
                        "조치후_취약" : [
                            ...
                            "~ㅇ~",
                            "구글 Play 프로텍트를 조치하는걸 원치 않으신점을 고려하여 조치하지 않았습니다.\n 보다 안전한 모바일 기기 사용을 위해 구글 Play 프로텍트를 활성화 하여 사용하시는것을 권고드립니다.",
                            "~ㅁ~",
                            "개발자 옵션이 활성화되어 있어 취약하시나, 고객님께서 조치 원해주시지 않아 조치를 진행하지 않았습니다.\n 보다 안전한 모바일 기기 사용을 위해 개발자 옵션을 비활성화 하시는것을 권고드립니다.",
                            "앱 권한 관리 부분 중 출처를 알 수 없는 앱 설치 권한이 부여되어있는 어플이 있으셨으나, 고객님꼐서 조치를 원하시지 않아 진행하지 않았습니다.\n 보다 안전한 모바일 기기 사용을 위해 출처를 알 수 없는 앱 설치 권한을 부여 해재후 사용하시는것을 권고드립니다.",
                            "~ㅅ~",
                        ],
            ...
            try:
                ...
                self.afterb_text = self.scripts["조치후_취약"]
            except KeyError:
            ...
            try:
                ...
                if len(self.afterg_text) < 8 and len(self.afterb_text) < 8:
                    log_writer('I',"Miss Match Script")
                    with open("./script.json", encoding="utf-8") as f:
                        data = json.load(f)
                    data["조치후_양호"] = [
                                "1번(백신 설치 여부 확인)과 2번항목(실시간 감시 및 최신 업데이트 여부 확인) 의 경우 점검전 설치가 되어있지 않았으나, 고객님께서 설치를 진행해주셨고, 실시간 감시 또한 활성화 되어 보다 안전하게 이용하실수 있는점 참고 부탁드립니다.",
        ...
        try:
            ...
            if self.clip_ck_box.isChecked():
                ...
                if idx == 1:
                    ...
                    if len(self.afterb_text[n]) >= 2 and type(self.afterb_text[n]) == list:
                        copy("\n".join(self.afterb_text[n]))
                    else:
                        copy(self.afterb_text[n])
